...
function [popcell] = LUCY_4_core(startdate,SP,F,N,S,W,E,WD,NOD,cityname,showimage,use_opt,opt_path,YYYYclose,YYYYtabdata,statreg,onlyurban,outputfolder)
% Inputdata
% DOY - day of year,
...
ahfresults=[];ahfLondon=[];datafolder='Data\DataGPW';tablefolder='Data';
...
% Textfiles (2005) of: Fixed public holidays, Metabolism and Traffic patterns
fixedpublicholidays=importdata([WD,filesep,tablefolder,filesep,'fixedpublicholidays.txt']);
metabolismhour=importdata([WD,filesep,tablefolder,filesep,'Metabolism.txt']);
weekdayhour=importdata([WD,filesep,tablefolder,filesep,'weekdayhours.txt']);
weekendhour=importdata([WD,filesep,tablefolder,filesep,'weekendhours.txt']);
weekendday=importdata([WD,filesep,tablefolder,filesep,'weekenddays.txt']);
...
energytable=importdata([WD,filesep,tablefolder,filesep,'Energy.txt']);
...
countrystatustable=importdata([WD,filesep,tablefolder,filesep,'CountriesEconomicStatus.txt']);
...
weightmatrix=importdata([WD,filesep,tablefolder,filesep,'ChangeEnergyperCDDHDD.txt']);
weightmatrix=weightmatrix.data;
noncooling=importdata([WD,filesep,tablefolder,filesep,'CountriesSummerCooling.txt']);
...
carstable=importdata([WD,filesep,tablefolder,filesep,'Cars.txt']);
...
freightstable=importdata([WD,filesep,tablefolder,filesep,'Freights.txt']);
...
mbikestable=importdata([WD,filesep,tablefolder,filesep,'Motorbikes.txt']);
...
for DOY=DOY:DOY+NOD-1
    ...
    avtemp=TemperatureWeighting_v5(DOY,N,S,W,E,WD,use_opt,opt_path,startdate,do,balancegrid,HDDgrid,CDDgrid,res);
...
%         save([foldername,filesep,ahffilename],'ahf','-ASCII');        
        dlmwrite([foldername,filesep,ahffilename],ahf, 'delimiter',' ','precision', '%.2f');
        index=index+1;
    end
    if H==24
        H=0;
